    ...
    def from_model_parts(
        ...
        *,
        ...
        for index, block in enumerate(blocks):
            ...
            if target is not None:
                ...
                entries.append(
                    _PartialGraphEntry(
                        ...
                        # Reentrant checkpointing invokes attention once under no-grad
                        # and once under grad-enabled recompute. Each contract needs its
                        # own TE graph because requires_grad is a replay invariant.
            ...
            if target is not None:
                if any(True for _ in target.parameters()):
                    raise RuntimeError(f"The partial MoE preprocess boundary in {block.name} must be parameterless")
                entries.append(
                    _PartialGraphEntry(
