...
class _Model(nn.Module):
    def __init__(self, *, attention=True, router=True, preprocess=True, layer_limit=1):
        super().__init__()
        self.config = SimpleNamespace(model_type="test_moe")
        self.backend = SimpleNamespace(
            partial_cuda_graph_attention=attention,
            partial_cuda_graph_moe_router=router,
            partial_cuda_graph_moe_preprocess=preprocess,
            partial_cuda_graph_layer_limit=layer_limit,
        )
        self.model = nn.Module()
        self.model.layers = nn.ModuleDict({"0": _Layer()})
...
def test_rejects_pipeline_parallel():
    with pytest.raises(RuntimeError, match="pipeline parallel size 1"):
        partial_graphs.PartialCudaGraphManager.from_model_parts([_Model()], pipeline_parallel=True)


def test_rejects_layer_limit_larger_than_discovered_stack():
    with pytest.raises(RuntimeError, match="exceeds the 1 discovered"):
        partial_graphs.PartialCudaGraphManager.from_model_parts([_Model(layer_limit=2)])
